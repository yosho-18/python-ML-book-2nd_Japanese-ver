...
# データセットの列を標準化し，ロジスティック回帰といった線形分類器に入力する，さらに主成分分析(PCA)を使用する（30次元を2次元の部分空間に圧縮）
# これらの操作をパイプラインで結合する
...
for k, (train, test) in enumerate(kfold):
    ...
    score = pipe_lr.score(X_train[test], y_train[test])  # X_train[[0 1 ... 44]]:0から44番目のX_trainを取り出す
    scores.append(score)
